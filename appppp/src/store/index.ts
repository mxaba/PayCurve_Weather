import { LogLatCoordinates, CurrentWeatherType, DailyWeatherType, Timezone } from '@/allTypes';
...
export default createStore({
  ...
  actions: {
    ...
    async getDailyWeather({ commit }, { lat, lon }: LogLatCoordinates) {
      const { data } = await axios.get(
    ...
    async getTimezone({ commit }, { lat, lon }: LogLatCoordinates) {
      const { data } = await axios.get(
